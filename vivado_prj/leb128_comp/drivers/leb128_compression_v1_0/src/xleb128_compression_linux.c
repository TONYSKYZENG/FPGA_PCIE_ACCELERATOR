...
typedef struct {
    ...
    XLeb128_compression_uio_map maps[ MAX_UIO_MAPS ];
} XLeb128_compression_uio_info;

/***************** Variable Definitions **************************************/
static XLeb128_compression_uio_info uio_info;
...
static int uio_info_read_name(XLeb128_compression_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XLeb128_compression_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XLeb128_compression_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XLeb128_compression_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XLeb128_compression_Initialize(XLeb128_compression *InstancePtr, const char* InstanceName) {
	XLeb128_compression_uio_info *InfoPtr = &uio_info;

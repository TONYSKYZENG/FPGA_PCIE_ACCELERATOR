...
// ==============================================================
`timescale 1ns/1ps
module leb128_compression_CTRL_BUS_s_axi
#(parameter
    ...
    output wire [31:0]                   wordsIn,
    input  wire [31:0]                   bytesOut,
    input  wire                          bytesOut_ap_vld,
...
localparam
    ...
    ADDR_WORDSIN_DATA_0  = 6'h28,
    ADDR_WORDSIN_CTRL    = 6'h2c,
    ADDR_BYTESOUT_DATA_0 = 6'h30,
    ADDR_BYTESOUT_CTRL   = 6'h34,
    WRIDLE               = 2'd0,
    WRDATA               = 2'd1,
    WRRESP               = 2'd2,
    WRRESET              = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_wordsIn = 'b0;
    reg                           int_bytesOut_ap_vld;
    reg  [31:0]                   int_bytesOut = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_WORDSIN_DATA_0: begin
                    rdata <= int_wordsIn[31:0];
                end
                ADDR_BYTESOUT_DATA_0: begin
                    rdata <= int_bytesOut[31:0];
                end
                ADDR_BYTESOUT_CTRL: begin
                    rdata[0] <= int_bytesOut_ap_vld;
                end
            endcase
        end
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_WORDSIN_DATA_0)
            int_wordsIn[31:0] <= (WDATA[31:0] & wmask) | (int_wordsIn[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_bytesOut_ap_vld <= 1'b0;
    else if (ACLK_EN) begin
        if (bytesOut_ap_vld)
            int_bytesOut_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_BYTESOUT_CTRL)
            int_bytesOut_ap_vld <= 1'b0; // clear on read
    end
end
